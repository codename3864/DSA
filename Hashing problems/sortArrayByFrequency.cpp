...
case contains a single integer N denoting the size of array. The second line contains N space-separated integers A1, A2, ..., AN denoting the elements
 of the array.
...
Testcase2: The highest frequency here is 3. The element 9 has the highest frequency. So 9 9 9 comes first.
...
void sortByFrequency(int *arr,int n) {
    ...
    for(; it != mp.end(); it++) {
        vr.push_back({it->first,it->second});
    }
    sort(vr.begin(),vr.end(),sortBySecond);
    vector <int> res;
    for(int i = 0; i < vr.size(); i++) {
        for(int j = 0 ; j < vr[i].second; j++) {
            res.push_back(vr[i].first);
        }
    }
